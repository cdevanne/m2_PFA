...
TRandom *R = new TRandom(); //out of the function because this is pseudo-random, and the first results is always the same with same parameters
Cell::Cell(int xId, int yId, int zId, double x, double y, double z, int particleId, int thr, double time)
{
	...
	double mean = time;
	double sigma = .1;	// +- 100ps

	R->Gaus(mean, sigma); R->Gaus(mean, sigma);
	timing = R->Gaus(mean, sigma);
}
