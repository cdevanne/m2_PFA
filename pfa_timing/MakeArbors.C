...
std::vector<Arbor*> FusionArbor(std::vector<Arbor*> arbors, double maxDistance, int minSize, int maxSize, double timing, double timingMin)
{
	...
	IdentifieChargedParticle(arbors, 1);
...
std::vector<Arbor*> FinalRecombination(std::vector<Arbor*> arbors, double energyMax, double rangeParam) 
{
	IdentifieChargedParticle(arbors, 1);
	...
	while (size != oldSize)
	{
		...
		{	
			...
			double energy = TotalEnergy(arbors[i], arbors[id]);
			double distance = arbors[id]->Distance(arbors[i], 1., 1., 0.025);

			if (distance < range && arbors[i]->IsCharged() == false)
			{
				if (arbors[id]->IsCharged() == false)
				{
					arbors[id]->SetArborId(i);
					range = distance;
				}else
				{
					if (energy < energyMax+sqrt(energyMax)*0.5 )
					{
						arbors[id]->SetArborId(i);
						range = distance;
					}
	...
	if (NumberOfArborInSize(arbors, 10, 10000) < 3) return arbors;

	id =  arborsMaxSizeTop3(arbors)[1];

	if (arbors[id]->GetSize() < 25) return arbors;


	for (int i = 0; i < arbors.size(); ++i)
	{	
		if(i == id) continue;
		if (arbors[id]->IsCharged() == false) range = rangeParam * 1.5;
			else range = rangeParam;
	...
	if (NumberOfArborInSize(arbors, 10, 10000) < 4) return arbors;
...
std::vector<Arbor*> MakeArbors(std::vector<Arbor*> branches, std::vector<Cluster*> trace)
{
	...
	double timingMin = .0031;
	double timingMax = .02;


	
	
	branches = RepairTimingFragmentation(branches, 30.);
	...
	{
		oldSize = size;
		arbors = FusionArbor(arbors, 35., 5, 1000, timingMax, timingMin);
		size = arbors.size();
	}
	...
	{
		oldSize = size;
		arbors = ClusteringSmallArbor(arbors, 4, 8, 40., timingMax, timingMin);
		size = arbors.size();
	}
	...
	{
		oldSize = size;
		arbors = FusionArbor(arbors, 20., 10, 1000, timingMax, timingMin);
		size = arbors.size();
	}
	...
	{
		oldSize = size;
		arbors = ClusteringSmallArbor(arbors, 10, 10, 80., timingMax, timingMin);
		size = arbors.size();
	}
	...
	{
		oldSize = size;
		arbors = FusionArborFromSeed(arbors, 25., 10, 1000, timingMax, timingMin);
		size = arbors.size();
	}

	arbors = CommonSeedArbor(arbors, 25., 1, 80);
	arbors = BuildArborSeedMissing(arbors, true, 3, 15., 10, 10000, timingMax, timingMin);
	arbors = BuildArborSeedMissing(arbors, false, 3, 15., 10, 10000, timingMax, timingMin);

		

	arbors = FinalRecombination(arbors, 30., 30.);
	...
	{
		oldSize = size;
		arbors = ClusteringSmallArbor(arbors,30, 30, 120., timingMax, timingMin);
		size = arbors.size();
	}
	...
	arbors = FinalRecombination(arbors, 30., 75.);
	arbors = FinalRecombination(arbors, 30., 100.);
	
	arbors = FinalRecombination(arbors, 10000., 30.);
